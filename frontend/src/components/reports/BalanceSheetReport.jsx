  ...
  return (
    <div className="space-y-6">
      <div className="text-center pb-6 border-b-2 border-slate-900">
        ...
        {asOfDate && <p className="text-sm text-slate-600 mt-2">As of {formatDisplayDate(asOfDate)}</p>}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Assets */}
        <div>
          <h3 className="text-lg font-semibold text-slate-900 mb-4 pb-2 border-b-2 border-slate-300">Assets</h3>
        ...
        {/* Liabilities & Equity */}
        <div>
          <h3 className="text-lg font-semibold text-slate-900 mb-4 pb-2 border-b-2 border-slate-300">Liabilities & Equity</h3>
          
          {/* Liabilities */}
          <div className="mb-6">
            <h4 className="font-semibold text-slate-700 mb-2 text-sm">Liabilities</h4>
            ...
            <div className="flex justify-between py-2 px-4 bg-slate-100 rounded text-sm">
              <span className="font-semibold text-slate-700">Total Liabilities</span>
              <span className="font-bold text-slate-900">{formatCurrency(data.totalLiabilities || 0)}</span>
            </div>
          </div>

          {/* Equity */}
          <div className="mb-6">
            <h4 className="font-semibold text-slate-700 mb-2 text-sm">Equity</h4>
            <div className="space-y-1 mb-2">
            ...
            <div className="flex justify-between py-2 px-4 bg-slate-100 rounded text-sm">
              ...
              <span className="font-bold text-slate-900">{formatCurrency(data.totalEquity || 0)}</span>
            </div>
          </div>
      ...
      {/* Balance Check */}
      <div className={`py-3 px-4 rounded-lg text-center font-semibold ${isBalanced ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
